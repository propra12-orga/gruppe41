...
public class Map {
	...
	/**
	 * A vector containing all map objects (map object is an abstract class). Players
	 * are included.
	 */
	public Vector<MapObject> objects = new Vector<MapObject>();

	/**
	 * Creates a new map, using the MapParse class to get a map from an XML
	 * file.
	 * 
	 * @param name
	 *            - Map name.
	...
	public void Render(Graphics2D g) {
		...
		for (int y = 0; y < TILES_COUNT_Y; y++) {
			for (int x = 0; x < TILES_COUNT_X; x++) {
				g.drawImage(tile_ground, BORDER_LEFT_WIDTH + (x * TILE_SIZE),
						BORDER_TOP_HEIGHT + (y * TILE_SIZE), BORDER_LEFT_WIDTH
								+ (x * TILE_SIZE) + TILE_SIZE,
						BORDER_TOP_HEIGHT + (y * TILE_SIZE) + TILE_SIZE,
						(ground[y][x] * TILE_SIZE), 0,
						(ground[y][x] * TILE_SIZE) + TILE_SIZE, TILE_SIZE, null);
			}
		}
	...
	public MapObject[] getBlockingObjects(Moveable m, int x, int y) {
		...
		for (MapObject o : objects) {
			if ((o == m) || (m.x + m.width > o.x) && (o.x + o.width > m.x)
					&& (m.y + m.height > o.y) && (o.y + o.height > m.y))
				continue;
			else if ((x + m.width > o.x) && (o.x + o.width > x)
					&& (y + m.height > o.y) && (o.y + o.height > y)) {
				int side;
