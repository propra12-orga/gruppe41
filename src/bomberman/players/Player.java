...
public class Player extends Mob
{
	...
	/**
	 * Only used in single player mode. If this boolean is true, the game ends with a win-message and a menu.
	 */
	...
	public Player(Input input, Map map, int tile_x, int tile_y, int player)
	{
		...
		BufferedImage temp = Image.read("data/sprites/player_" + player + ".png");
	...
	/**
	 * Update method for player. Moves the player if necessary and plants bombs. Also updates th players animation.
	 */
	public void Update()
	{
		if (moving)
	...
	/**
	 * Draws the player The used animation depends on the moving boolean and the face direction.
	 */
	public void Render(Graphics2D g)
	{
		if (!moving)
		{
			g.drawImage(img, x, y + Map.TILE_SIZE - HEIGHT - 1, x + Map.TILE_SIZE, y + Map.TILE_SIZE - 1, 0, dir * HEIGHT, WIDTH, dir * HEIGHT + HEIGHT, null);
		}
		else
	...
	/**
	 * Creates a new bomb. Does not have any parameters. Position on the field is just the player's position.
	 */
	...
	/**
	 * Blocking method, usually returns false, returns true if the other object is a bomb.
	 */
	...
	/**
	 * Spawning the player increases the number of players saved on the map. Also ses movement speed to one.
	 */
	public void OnSpawn()
	{
	...
	/**
	 * Die method, called up by onHurt method. The player is removed and a burnt player is shown. Doesn't work properly.
	 */
	public void Die()
	{
		super.Die();
